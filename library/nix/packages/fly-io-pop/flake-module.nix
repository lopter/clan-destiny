  ...
  perSystem =
    { lib, pkgs, inputs', system, ... }:
    {
      ...
      let
        ...
        usergroups = with destiny-config.lib.usergroups; dockerNssHelpers (withGroupBase groups) (withUserBase users);
        result = lib.nixos.evalModules {
          modules = [
            ...
            "${self.inputs.nixpkgs}/nixos/modules/misc/assertions.nix"
            {
              ...
              sops.secrets.nginxVaultAgentRoleIdPath = { owner = "nginx"; group = "nginx"; };
              sops.secrets.nginxVaultAgentSecretIdPath = { owner = "nginx"; group = "nginx"; };
              sops.secrets.vaultTLSCACert.mode = "0444";
            }
          ];
          specialArgs = { inherit lib pkgs self system; };
        };
