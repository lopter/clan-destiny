            ...
            config =
              { pkgs, ... }:
              {
                ...
                systemd.services."syncthing-init-remove-default-folder" = {
                  ...
                  path = with pkgs; [ bash libxml2 curl ];
                  # TODO: set some PATH with libxml2's xmllint and curl
                  # And also pass the runtime directory with the api key
                  serviceConfig = {
                    RemainAfterExit = true;
                    RuntimeDirectory = "syncthing-init-remove-default-folder";
                    Type = "oneshot";
                    ExecStart = "${../../bash/syncthing-init-remove-default-folder} ${lib.escapeShellArg name}";
                  };
                };
