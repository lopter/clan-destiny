...
in
{
  options.clan-destiny.syncthing = with lib; {
    createUserAccounts = mkOption {
      ...
      description = ''
        For each listed user create a Syncthing DeviceId and run Syncthing in a
        container.
      '';
      default = [ ];
    };
    ...
    guiVirtualHost = mkOption {
      type = with types; nullOr nonEmptyStr;
      description = ''
        The syncthing GUI for each user will be exposed from the host at this
        virtual host using Nginx.
      '';
      default = "syncthing-${zone}.${domain}";
    };
  };

  config = lib.mkIf ((builtins.length cfg.createUserAccounts) > 0) {
    assertions = [
      {
          ...
          config =
            { pkgs, ... }:
            {
              ...
              services.syncthing =
              {
                ...
                settings = {
                  gui = {
                    # apiKey = ; # can't be set to a path, but could be passed as a flag through a wrapper
    ...
    # standalone module.
    services.nginx.virtualHosts.${cfg.guiVirtualHost} =
    let
