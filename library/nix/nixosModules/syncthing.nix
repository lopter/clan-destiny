...
let
  ...
  inherit (self.packages.${system})
    nginxWithPamSupport
    syncthing-init-ignore-service
    syncthing-init-remove-default-folder
    ;
...
in
{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf ((builtins.length cfg.createUserSystemInstances) > 0) {
              ...
              { ... }:
              {
                ...
                systemd.services."syncthing-init-ignores-${name}" = {
                  ...
                  serviceConfig = {
                    ...
                    Type = "oneshot";
                    ExecStart = "${lib.getExe syncthing-init-ignore-service} ${lib.escapeShellArg name}";
                  };
                };
                systemd.services."syncthing-init-remove-default-folder" = {
                  description = "Remove the Default Folder generated by syncthing";
                  after = [ "syncthing.service" ];
                  wantedBy = [ "multi-user.target" ];
                  serviceConfig = {
                    RemainAfterExit = true;
                    RuntimeDirectory = "syncthing-init-remove-default-folder";
                    Type = "oneshot";
                    ExecStart = "${lib.getExe syncthing-init-remove-default-folder} ${lib.escapeShellArg name}";
                  };
                };
      ...
      '';
      services.nginx.package = nginxWithPamSupport;
